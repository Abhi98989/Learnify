...
function AjaxCallWithoutData(url) {
    return AjaxCall(url).then((result) => {
        return result;
    });
};
...
async function AjaxCallNoLoader(url, data = {}) {
    var Dropdown = JSON.stringify(data);
    var result;
    await $.ajax({
        type: "Post",
...
function AjaxCallWithoutDataNoLoader(url) {
    return AjaxCallNoLoader(url).then((result) => {
        return result;
    });
};

function AjaxCallNoAsyncNoLoader(url, data = {}) {
    var Dropdown = JSON.stringify(data);
    var result;
    $.ajax({
        type: "Post",
...
function SweetAlert(msgType, msg) {
    Swal.fire({
        position: 'top-end',
        icon: msgType,
        title: msg,
